...
int main(){
   ProcessTable PT ;
   Process p1 = Process(PT.get_pid(), "process_1", 30, "run" , 5, 2, 3000, 1);
   
   // add p1 to PT
   PT.add_process(p1);
   // create p2
   Process p2 = Process(PT.get_pid(), "process_2", 30, "run" , 5, 2, 3000, 1);
   // addd p2 to PT
   PT.add_process(p2);
 ...
 /*   Process p1= Process( 1005, "process_1", 30, "run" , 5, 2, 3000, 1); */
    ...
    PT.remove_process(p1);
    // remove process p1 from table
    PT.remove_process(p2);
